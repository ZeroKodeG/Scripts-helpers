...
- `auditoria_sistema.bat` — module 1/3: system info, local accounts, firewall/RDP/UAC registry state, services, running processes, group policy, domain/DC connectivity, hardware inventory. Writes `Reporte_Sistema_CMD.txt` to `%TEMP%` and a parallel debug log `Auditoria_Sistema_LOG.txt`.
- `auditoria_red.bat` — module 2/3: network config, routing table, listening ports/established connections (`netstat`), ARP/NetBIOS neighbors, gateway/DNS connectivity checks. Writes `Reporte_Red_CMD.txt` to `%TEMP%`.
- `auditoria_logs.bat` — module 3/3: Windows Security/System event log queries (`wevtutil`) for failed logons (4625), successful logons (4624), RDP logons (4624 type 10), privilege use (4672), new services (4697/7045), account/group changes (4720/4732), log-clearing events (1102, a tamper indicator), Kerberos pre-auth failures (4771), unexpected shutdowns (6008/41). Writes `Reporte_Logs_CMD.txt` to `%TEMP%`.
- `ejecutar_auditoria.bat` — orchestrator: downloads the 3 module scripts fresh from the backend's own `/scripts/` route into `%TEMP%`, runs them with `/silent`, then POSTs the 3 resulting `.txt` files from `%TEMP%` to `POST /api/reportes` using an `X-API-Key` header. The key must belong to an **active admin** user in Postgres. Reads `API_KEY`/`API_URL` from env vars (`AUDIT_API_KEY`/`AUDIT_API_URL`) or `%TEMP%\.audit_config`. Designed to run as a scheduled task under SYSTEM; `/silent` skips every `pause`. The API key is only ever used for the upload step, never for downloading the scripts (that route has no auth by design — it holds no secrets).
...
- `/debug` (`auditoria_sistema.bat` only) — prints extra diagnostics (line count, `errorlevel` after each section) and keeps the temp report file instead of deleting it after copying to `%TEMP%`; useful when antivirus (the scripts specifically call out Kaspersky) truncates the `.bat` file, since a mismatched line count reveals that.

Report generation pattern: each module writes to a randomized temp file (`%TEMP%\Audit<Name>_%COMPUTERNAME%_%RANDOM%.txt`) via a `:seccion` subroutine, then copies the result to a fixed filename in `%TEMP%`. This avoids partial/locked-file issues if a previous report is open in Notepad, and works when the process is SYSTEM. Manual and scheduled-task invocations use the same PowerShell WebClient download into `%TEMP%` plus process env vars `AUDIT_API_KEY`/`AUDIT_API_URL`; the scheduled task adds `/silent`. Examples are in `README.md`.
